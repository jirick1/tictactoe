...
var ViewController = {
	...
	/**
	@function updates the UI when the users clicks 
	          on a particular cell in the 3x3 grid
	@param {Table>Tr>td on click event}
	*/
	...
	*/
	...
	__updateScore: function(character)
	{
		if (character == 'x')
		{
			View.updateUI('x_score', parseInt(document.getElementById('x_score').innerHTML)+1);
		}
		else
		{
			View.updateUI('o_score', parseInt(document.getElementById('o_score').innerHTML)+1);
		}
	},
	...
	*/
	_isNotEmpty: function(element)
	{
		return (element.currentTarget.innerHTML.indexOf("x.png") != -1 
			|| element.currentTarget.innerHTML.indexOf("o.png") != -1);
	},

	
	/**
	@function check to see which image is in the table cell
	@param {id of the table cell}
	*/
	...
	*/
	aiPlaceLetter: function()
	{
		...
		*/
		...
		else if (!rowOneColumnOne) { View.updateUI("r1c1", image);}
		else if (!rowOneColumnThree) { View.updateUI("r1c3", image); }
		else if (!rowThreeColumnOne) { View.updateUI("r3c1", image); }
		else if (!rowThreeColumnThree) { View.updateUI("r3c3", image); }
